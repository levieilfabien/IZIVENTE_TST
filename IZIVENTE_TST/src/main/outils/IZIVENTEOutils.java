	...
	public static boolean murissement(String siocID, int distributeur, boolean creditRenouvelable, String dateSaisie) {
		...
		try {
			...
			if (creditRenouvelable) {
				...
				SQL = "update [IZIBOX].[dbo].[SIMULATION_OPC_CONTRAT] set SIOC_DATE_FIN_DELAI_RETRA = '" + nouvelleDate + "' where sioc_id = " + siocID;
			} else {
				// Pour les crédit personnels on modifie la date de financement
				SQL = "update [IZIBOX].[dbo].FINANCEMENT set fina_date = '" + nouvelleDate + "' where sioc_id = " + siocID + " and FINA_RANG_FINANCEMENT = 1;";
			}
			modification += st.executeUpdate(SQL);
