...
public void accesIzivente() throws SeleniumException {
	...
	scenario11.setAlm(true);
	scenario11.setIdUniqueTestLab(54398);
	scenario11.setNomCasEssai("TNRSC11-" + getTime());
	scenario11.setDescriptif("TNRSC11 - BP - IZIVENTE_Editique Prêt étudiant différé partiel BPI");
	scenario11.setNomTestLab("TNRSC11 - BP - IZIVENTE_Editique Prêt étudiant différé partiel BPI");
	//scenario11.setNomTestPlan("TNRSC11 - BP - IZIVENTE_Editique Prêt étudiant différé partiel BPI");
	scenario11.setCheminTestLab("POC Selenium\\IZIVENTE");
	...
	String repertoire = creerRepertoireTelechargement(scenario11, profile);
	scenario11.setRepertoireTelechargement(repertoire);
	// Initialisation du driver
	FirefoxImpl driver = new FirefoxImpl(ffBinary, profile);
	driver.get(Constantes.URL_BP_FUTURE_REROUTAGE);
	// LISTE DES OBJECTIFS DU CAS DE TEST
	scenario11.ajouterObjectif(new ObjectifBean("Test arrivé à terme", scenario11.getNomCasEssai() + scenario11.getTime()));

    SeleniumOutils outil = new SeleniumOutils(driver, GenericDriver.FIREFOX_IMPL);
    outil.setRepertoireRacine(scenario11.getRepertoireTelechargement());
    ...
    try {
		...
		scenario11.getTests().add(CT01Initialisation(scenario11, outil));
		scenario11.getTests().add(CT02OuvertureDossier(scenario11, outil));
		scenario11.getTests().add(CT03SaisieDossier(scenario11, outil));
		scenario11.getTests().add(CT04Participants(scenario11, outil));
		scenario11.getTests().add(CT05FinalisationInstruction(scenario11, outil));
		
	} catch (SeleniumException ex) {
		// Finalisation en erreur du cas de test.
		finaliserTestEnErreur(outil, scenario11, ex, scenario11.getNomCasEssai() + scenario11.getDateCreation().getTime());
		throw ex;
	}
	// Finalisation normale du cas de test.
	finaliserTest(outil, scenario11, scenario11.getNomCasEssai() + scenario11.getDateCreation().getTime());
}


/**
 * Partie du scenario11 regroupant la saisie, l'instruction et la validation d'un dossier crédit amortissable PP pour prêt étudiant différé partiel BPI dans IZIVENTE BP.
...
public CasEssaiIziventeBean CT03SaisieDossier(CasEssaiIziventeBean scenario, SeleniumOutils outil) throws SeleniumException {
	...
	outil.selectionner("BPI ETUDIANT ECH DIF PART", Cibles.SELECTEUR_OFFRE_CREDIT, false);
	...
	outil.selectionner("ECOLE INGENIEURS", Cibles.SELECTEUR_OBJET_FINANCE);
	outil.attendreChargementElement(Cibles.SAISIE_COUT_PROJET);
	outil.viderEtSaisir("4000", Cibles.SAISIE_COUT_PROJET);
	outil.viderEtSaisir("12", Cibles.SAISIE_DUREE_DIFFERE_PARTIEL);
	outil.viderEtSaisir("4000", Cibles.SAISIE_MONTANT_DEMANDE);
	outil.viderEtSaisir("40", Cibles.SAISIE_DUREE_DEMANDE);
