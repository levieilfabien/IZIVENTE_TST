 ...
 */
public class TNRSC09 extends SC00Test {
...
public void accesIzivente() throws SeleniumException {
	...
	scenario9.setAlm(true);
	scenario9.setIdUniqueTestLab(54396);
	scenario9.setNomCasEssai("TNRSC09-" + getTime());
	scenario9.setDescriptif("TNRSC09 - CE - IZIVENTE_Editique Prêt étudiant echelonné différé total FEI");
	scenario9.setNomTestLab("TNRSC09 - CE - IZIVENTE_Editique Prêt étudiant echelonné différé total FEI");
	//scenario9.setNomTestPlan("TNRSC09 - IZIVENTE_Editique Prêt étudiant echelonné différé total FEI");
	scenario9.setCheminTestLab("POC Selenium\\IZIVENTE");
	...
	String repertoire = creerRepertoireTelechargement(scenario9, profile);
	scenario9.setRepertoireTelechargement(repertoire);
	// Initialisation du driver
	FirefoxImpl driver = new FirefoxImpl(ffBinary, profile);
	driver.get(Constantes.URL_CE_FUTURE_REROUTAGE);
	// LISTE DES OBJECTIFS DU CAS DE TEST
	scenario9.ajouterObjectif(new ObjectifBean("Test arrivé à terme", scenario9.getNomCasEssai() + scenario9.getTime()));
   
    SeleniumOutils outil = new SeleniumOutils(driver, GenericDriver.FIREFOX_IMPL);
    outil.setRepertoireRacine(scenario9.getRepertoireTelechargement());
    ...
    try {
		...
		scenario9.getTests().add(CT01Initialisation(scenario9, outil));
		scenario9.getTests().add(CT02OuvertureDossier(scenario9, outil));
		scenario9.getTests().add(CT03SaisieDossier(scenario9, outil));
		scenario9.getTests().add(CT04Participants(scenario9, outil));
		scenario9.getTests().add(CT05FinalisationInstruction(scenario9, outil));
		
	} catch (SeleniumException ex) {
		// Finalisation en erreur du cas de test.
		finaliserTestEnErreur(outil, scenario9, ex, scenario9.getNomCasEssai() + scenario9.getDateCreation().getTime());
		throw ex;
	}
	// Finalisation normale du cas de test.
	finaliserTest(outil, scenario9, scenario9.getNomCasEssai() + scenario9.getDateCreation().getTime());
}


/**
 * Partie du scenario9 regroupant la saisie, l'instruction et la validation d'un dossier crédit amortissable PP pour prêt étudiant echelonné différé total FEI dans IZIVENTE CE.
...
public CasEssaiIziventeBean CT03SaisieDossier(CasEssaiIziventeBean scenario, SeleniumOutils outil) throws SeleniumException {
	...
	outil.attendre(1);
	outil.attendreChargementElement(Cibles.SELECTEUR_OBJET_FINANCE);
	outil.selectionner("AUTRE ETUDE", Cibles.SELECTEUR_OBJET_FINANCE);
	outil.attendreChargementElement(Cibles.SAISIE_COUT_PROJET);
	outil.viderEtSaisir("2000", Cibles.SAISIE_COUT_PROJET);
	outil.viderEtSaisir("12", Cibles.SAISIE_DUREE_DIFFERE_PARTIEL);
	outil.viderEtSaisir("2000", Cibles.SAISIE_MONTANT_DEMANDE);
	outil.viderEtSaisir("20", Cibles.SAISIE_DUREE_DEMANDE);
