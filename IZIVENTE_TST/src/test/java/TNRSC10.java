 ...
 */
public class TNRSC10 extends SC00Test {
...
public void accesIzivente() throws SeleniumException {
	...
	scenario10.setAlm(true);
	scenario10.setIdUniqueTestLab(54397);
	scenario10.setNomCasEssai("TNRSC10-" + getTime());
	scenario10.setDescriptif("TNRSC10 - BP - IZIVENTE_Editique Prêt Etudiant");
	scenario10.setNomTestLab("TNRSC10 - BP - IZIVENTE_Editique Prêt Etudiant");
	//scenario10.setNomTestPlan("TNRSC10 - BP - IZIVENTE_Editique Prêt Etudiant");
	scenario10.setCheminTestLab("POC Selenium\\IZIVENTE");
	...
	String repertoire = creerRepertoireTelechargement(scenario10, profile);
	scenario10.setRepertoireTelechargement(repertoire);	
	// Initialisation du driver
	FirefoxImpl driver = new FirefoxImpl(ffBinary, profile);
	driver.get(Constantes.URL_BP_FUTURE_REROUTAGE);
	// LISTE DES OBJECTIFS DU CAS DE TEST
	scenario10.ajouterObjectif(new ObjectifBean("Test arrivé à terme", scenario10.getNomCasEssai() + scenario10.getTime()));
   
    SeleniumOutils outil = new SeleniumOutils(driver, GenericDriver.FIREFOX_IMPL);
    outil.setRepertoireRacine(scenario10.getRepertoireTelechargement());
    ...
    try {
		...
		scenario10.getTests().add(CT01Initialisation(scenario10, outil));
		scenario10.getTests().add(CT02OuvertureDossier(scenario10, outil));
		scenario10.getTests().add(CT03SaisieDossier(scenario10, outil));
		scenario10.getTests().add(CT04Participants(scenario10, outil));
		scenario10.getTests().add(CT05FinalisationInstruction(scenario10, outil));
		
	} catch (SeleniumException ex) {
		// Finalisation en erreur du cas de test.
		finaliserTestEnErreur(outil, scenario10, ex, scenario10.getNomCasEssai() + scenario10.getDateCreation().getTime());
		throw ex;
	}
	// Finalisation normale du cas de test.
	finaliserTest(outil, scenario10, scenario10.getNomCasEssai() + scenario10.getDateCreation().getTime());
}

/**
 * Partie du scenario10 regroupant la saisie, l'instruction et la validation d'un dossier crédit amortissable PP pour prêt étudiant dans IZIVENTE BP.
